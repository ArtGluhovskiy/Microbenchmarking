...
public class ForkJoinCountedCompleterVsRecursiveBenchmark {
    ...
    public Integer findMaxRecursiveTask() {
        CustomRecursiveMaxFinder task = new CustomRecursiveMaxFinder(array, 0, array.length);
        return task.compute();
    }

    @Benchmark
    public Integer findMaxCountedCompleterTask() {
        CountedCompleterMaxFinder maxFinder = new CountedCompleterMaxFinder(null, array, 0, array.length);
        return maxFinder.invoke();
    }
}
